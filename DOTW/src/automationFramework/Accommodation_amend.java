...
//Login
	...
	public void Amend(String browsername) throws Exception {
		...
		try {
			...
			driver.findElement(Operations.chooseCust).sendKeys("maintenance3cc");
			Thread.sleep(2000);
			...
			action.sendKeys(Keys.ARROW_DOWN).build().perform();
			action.sendKeys(Keys.ENTER).build().perform();*/
			driver.findElement(NewAccoBooking.AccomUnit).sendKeys(excel.getData(0, 10, 0));
			Thread.sleep(4000);
			action.sendKeys(Keys.ARROW_DOWN).build().perform();
			action.sendKeys(Keys.ENTER).build().perform();

			driver.findElement(NewAccoBooking.inDate).clear();
			driver.findElement(NewAccoBooking.inDate).sendKeys(excel.getData(0, 13, 0));

			driver.findElement(NewAccoBooking.outDate).clear();
			driver.findElement(NewAccoBooking.outDate).sendKeys(excel.getData(0, 13, 1));
			...
			CharSequence cs = excel.getData(0, 10, 0);
			...
			if (result.contains(cs)) {
				...
				title.selectByVisibleText(excel.getData(2, 15, 1));

				Thread.sleep(1000);
				driver.findElement(NewAccoBooking.paxFname).sendKeys(excel.getData(2, 16, 1));
				Thread.sleep(1000);
				driver.findElement(NewAccoBooking.paxLname).sendKeys(excel.getData(2, 17, 1));
				...
				driver.findElement(PaymentPage.fName).sendKeys(excel.getData(2, 20, 1));

				driver.findElement(PaymentPage.LName).clear();
				driver.findElement(PaymentPage.LName).sendKeys(excel.getData(2, 21, 1));

				driver.findElement(PaymentPage.address).clear();
				driver.findElement(PaymentPage.address).sendKeys(excel.getData(2, 22, 1));

				driver.findElement(PaymentPage.zip).clear();
				driver.findElement(PaymentPage.zip).sendKeys(excel.getData(2, 23, 1));
				...
				driver.findElement(PaymentPage.city).sendKeys(excel.getData(2, 25, 1));

				Select ccType = new Select(driver.findElement(PaymentPage.ccType));
				ccType.selectByVisibleText(excel.getData(2, 26, 1));

				driver.findElement(PaymentPage.ccNum).clear();
				driver.findElement(PaymentPage.ccNum).sendKeys(excel.getData(2, 27, 1));

				driver.findElement(PaymentPage.ccName).clear();
				driver.findElement(PaymentPage.ccName).sendKeys(excel.getData(2, 28, 1));

				Select expMonth = new Select(driver.findElement(PaymentPage.expMonth));
				expMonth.selectByVisibleText(excel.getData(2, 29, 1));

				Select expYear = new Select(driver.findElement(PaymentPage.expYear));
				expYear.selectByVisibleText(excel.getData(2, 30, 1));

				driver.findElement(PaymentPage.cvv).clear();
				driver.findElement(PaymentPage.cvv).sendKeys(excel.getData(2, 31, 1));
				...
				if (ConfirmStatus.contains(cc)) {
					...
					driver.findElement(Amend.newFname).sendKeys(excel.getData(2, 35, 0));

					test.log(LogStatus.PASS, "Changed name");

					driver.findElement(Amend.newChckIn).clear();
					driver.findElement(Amend.newChckIn).sendKeys(excel.getData(2, 34, 0));

					driver.findElement(Amend.newChckOut).clear();
					driver.findElement(Amend.newChckOut).sendKeys(excel.getData(2, 34, 1));
					...
					String expectedchckIn=excel.getData(2, 37, 0);
					
					System.out.println(expectedchckIn);
					String expectedchckOut=excel.getData(2, 37, 1);
					String expectedFname=excel.getData(2, 35, 0);
