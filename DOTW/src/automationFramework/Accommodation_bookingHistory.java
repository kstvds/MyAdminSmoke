...
public class Accommodation_bookingHistory {
	...
	public void bookinghistory(String browsername) throws Exception {
		...
		try {
			...
			driverqa.findElement(Operations.company).sendKeys(excel.getData(4, 4, 0));
			Thread.sleep(1000);
		...
		try {
			...
			driverqa.findElement(NewAccoBooking.AccomUnit).sendKeys(excel.getData(4, 7, 0));
			Thread.sleep(2000);
			action.sendKeys(Keys.ARROW_DOWN).build().perform();
			action.sendKeys(Keys.ENTER).build().perform();
			driverqa.findElement(NewAccoBooking.inDate).clear();
			driverqa.findElement(NewAccoBooking.inDate).sendKeys(excel.getData(4, 10, 0));
			driverqa.findElement(NewAccoBooking.outDate).clear();
			driverqa.findElement(NewAccoBooking.outDate).sendKeys(excel.getData(4, 10, 1));
			String expected = excel.getData(4, 7, 0);
		...
		try {
			...
			driverqa.findElement(NewAccoBooking.paxFname).sendKeys(excel.getData(1, 14, 0));
			Thread.sleep(2000);
			driverqa.findElement(NewAccoBooking.paxLname).sendKeys(excel.getData(1, 14, 1));
