	  ...
	  public void search(String browsername) throws Exception {
		...
		logger.info("Searching Customer");
			 ...
			 try {
				 ...
				 driverqa.findElement(Operations.company).sendKeys(excel.getData(0, 1, 4));
				 Thread.sleep(1000);
			 ...
			 try{
				 ...
				 driverqa.findElement(NewAccoBooking.AccomUnit).sendKeys(excel.getData(0, 2, 7));
				 Thread.sleep(2000);
				 action.sendKeys(Keys.ARROW_DOWN).build().perform();
				 action.sendKeys(Keys.ENTER).build().perform();
				 driverqa.findElement(NewAccoBooking.inDate).clear();
				 driverqa.findElement(NewAccoBooking.inDate).sendKeys(excel.getData(0, 2, 8));
				 driverqa.findElement(NewAccoBooking.outDate).clear();
				 driverqa.findElement(NewAccoBooking.outDate).sendKeys(excel.getData(0, 2, 9));
				 String expectedhotel=excel.getData(0, 2, 7);
